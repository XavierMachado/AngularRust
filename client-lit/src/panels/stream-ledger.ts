...
import { LEDGER_WINDOW_MS, transport } from '../store/transport';
 ...
 * inbound are hollow.
 */
@customElement('wt-stream-ledger')
export class StreamLedger extends SignalWatcher(LitElement) {
  ...
  @query('canvas') private surface!: HTMLCanvasElement;
  ...
  private reading(): string {
    const ticks = transport.ticks.get();
    if (!ticks.length) {
    ...
    const lane = transport.datagramsEmulated.get() ? 'emulated datagrams' : 'datagrams';
      ...
      <figure class="ledger">
        <figcaption>
          ...
          <span class="key">
            ...
            <i class="swatch unreliable ${emulated ? 'emulated' : ''}"></i>
            ${emulated ? 'datagram lane, emulated' : 'datagrams, unreliable'}
          </span>
        </figcaption>
        <canvas aria-hidden="true"></canvas>
        <p class="reading">${this.reading()}</p>
      </figure>
    `;
  }
  ...
  override connectedCallback(): void {
    ...
    // Re-entering the page after a route change: `firstUpdated` already ran on
    // the first visit, so the loop restarts here.
    if (this.hasUpdated) {
      this.start();
    }
  }
  ...
  private start(): void {
    ...
    // is driven by the clock rather than by reactive updates. Anyone who asked
  ...
  private draw(): void {
    ...
    // Custom properties inherit across the shadow boundary, so the canvas can
    // still read the app's tokens off itself.
    ...
    for (const tick of transport.ticks.get()) {
