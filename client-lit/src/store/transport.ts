...
import { backoffMs, label, plan, type Preference } from '../core/negotiate';
import { DISCOVERY_URL, browserNet, type Net } from '../core/net';
import type {
  DatagramIn,
  DatagramOut,
  Discovery,
...
import { TauriIpcLink } from '../core/tauri-link';
import { validateSay } from '../core/wasm';
import { WebSocketLink } from '../core/websocket-link';
import { WebTransportLink } from '../core/webtransport-link';
import { computed, signal, update } from './reactive';
 ...
 *
 * A plain class over TC39 signals: where the Angular client injects this as a
 * service, here it is a module-level singleton and the routed pages all read
 * the same instance — which is what lets the session survive navigation.
 */
export class TransportStore {
  ...
  readonly online = computed(() => this.state.get() === 'online');
  readonly busy = computed(
    () => this.state.get() === 'connecting' || this.state.get() === 'closing',
  );

  readonly lastRtt = computed(() => {
    const samples = this.rtt.get();
    return samples.length ? samples[samples.length - 1].ms : null;
  });

  readonly medianRtt = computed(() => {
    const samples = this.rtt.get().map((sample) => sample.ms);
    if (!samples.length) {
      return null;
    }
  ...
  readonly lostDatagrams = computed(() => this.pingsSent.get() - this.rtt.get().length);
  ...
  constructor(
    ...
    this.ipcAvailable = net.tauriIpc !== null;
  }

  async connect(author: string): Promise<void> {
    if (this.state.get() === 'connecting' || this.state.get() === 'online') {
      return;
    }
  ...
  async ping(): Promise<void> {
    ...
    update(this.pingsSent, (sent) => sent + 1);
  ...
  /**
   * Records a line from elsewhere in the app. The global error hooks use
  ...
  private effectivePreference(): Preference {
    if (this.preference.get() === 'auto' && this.webTransportFailures >= DOWNGRADE_AFTER) {
  ...
  private requireLink(): Link {
    if (!this.link || this.state.get() !== 'online') {
  ...
  private applyPush(push: ServerPush): void {
    switch (push.kind) {
      case 'welcome':
        ...
        if (push.transport !== this.transport.get()) {
          this.write(
            'link',
            `The server calls this session ${push.transport}, not ${this.transport.get()}.`,
            'warn',
          );
        }
...
/**
 * The one store every page and panel reads. Living at module level — not in a
 * component — is what keeps the session, its log and its history alive across
 * route changes.
 */
export const transport = new TransportStore(browserNet(), DISCOVERY_URL);
